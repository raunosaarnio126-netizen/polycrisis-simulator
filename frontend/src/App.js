...
KEY FINDINGS:
${companyInsightsData.key_findings.map((finding, idx) => `${idx + 1}. ${finding}`).join('\n')}

${'='.repeat(80)}

RECOMMENDATIONS:
${companyInsightsData.recommendations.map((rec, idx) => `${idx + 1}. ${rec}`).join('\n')}
...
Generated by Polycrisis Simulator
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `${companyInsightsData.analysis_title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_${new Date().getTime()}.txt`;
...
AI ANALYSIS:
${currentAdjustment.real_time_analysis}

${'='.repeat(80)}

IMPACT SUMMARY:
${currentAdjustment.impact_summary}
...
RISK ASSESSMENT:
- Risk Level: ${currentAdjustment.risk_level.toUpperCase()}
- Created: ${new Date(currentAdjustment.created_at).toLocaleDateString()}
...
Generated by Polycrisis Simulator - Scenario Adjusters
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `scenario_adjustment_${currentAdjustment.adjustment_name.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_${new Date().getTime()}.txt`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    window.URL.revokeObjectURL(url);
...
Generated by Polycrisis Simulator - Scenario Adjusters
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `current_scenario_settings_${adjustmentName.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_${new Date().getTime()}.txt`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    window.URL.revokeObjectURL(url);
